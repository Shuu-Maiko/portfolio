import { INFO, BIOGRAPHY, EXPERIENCE, EDUCATION, SKILLS_INDEX } from "@/lib/data";
      ...
      <div className="container-publication">
        ...
        <div className="space-y-32">
          <section className="flex flex-col items-center gap-16 text-center">
            <div className="max-w-3xl space-y-10">
              {BIOGRAPHY.map((para, idx) => (
                <p key={idx} className="text-xl md:text-3xl leading-relaxed text-primary/90 font-serif">
                  {para}
                </p>
              ))}
            </div>
              ...
              {EXPERIENCE.map((item, idx) => (
                <div key={idx} className="flex flex-col gap-6 border-l border-primary/20 pl-10 relative">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[13px] font-bold text-primary/60 uppercase tracking-[0.2em]">{item.company}</p>
                  </div>
                  <p className="text-lg text-muted font-sans leading-relaxed opacity-90 max-w-2xl">{item.description}</p>
          ...
          <section className="space-y-16">
            ...
            <div className="max-w-3xl mx-auto border-l border-primary/20 pl-10 relative space-y-12">
               {EDUCATION.map((edu, idx) => (
                 <div key={idx} className="relative">
                   <div className="absolute top-2 left-[-40px] w-2.5 h-2.5 rounded-full bg-primary" />
                   <div className="space-y-6">
                     <div>
                       <h4 className="text-2xl font-serif font-normal text-primary">{edu.degree}</h4>
                       <p className="text-[13px] font-bold text-primary/60 uppercase tracking-[0.2em]">{edu.institution} &middot; {edu.period}</p>
                     </div>
                     {edu.curriculum && (
